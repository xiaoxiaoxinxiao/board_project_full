...
class BoardUI:
    ...
    def run_sign_recognition(self, frame_bgr):
        """
        288类 hand102 新模型：
        body_pose.rknn + hand_pose.rknn
        sign_tcn_288_hand102_64x102.rknn
        输入 [1, 64, 102]
        输出 [1, 288]
        """
        ...
        try:
            ...
            # 注意：make_46_feature 名字没改，但 realtime_demo.py 里已经让它返回 102维
            feat102 = make_46_feature(hand_xy, body_xy, frame_w, frame_h)

            if feat102 is None:
                self.sign_label.config(text="未检测到关键点")
                return
            ...
            x = np.expand_dims(x, axis=0)   # [1,64,102]

            slr_out = self.slr_rknn.inference(inputs=[x])

            if slr_out is None or len(slr_out) == 0:
                self.sign_label.config(text="SLR输出为空")
                return

            logits = np.asarray(slr_out[0])

            prob = softmax(logits).reshape(-1)

            # 禁用词和演示映射
            banned_labels = {125}       # 瘦 new=125
            label_remap = {
                168: 186,   # 老人 -> 我
                105: 99,    # 多 -> 有
                273: 80,    # 经验 -> 困难
            ...
            for cand in order[:5]:
                ...
                if cand in banned_labels:
                    continue

                pred = int(label_remap.get(cand, cand))
                score = float(prob[cand])
                break

            if pred is None:
                self.sign_label.config(text="候选全是禁用词，跳过")
                return

            word = self.id2word.get(pred, "")

            if score > 0.35 and word:
                self.last_word = word
